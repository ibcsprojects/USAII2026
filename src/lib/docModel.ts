...
export type ParagraphKind = 'normal' | 'bullet' | 'heading' | 'pageBreak' | 'table'

/** An inline image anchored in a paragraph. Sizes are the rendered dimensions in points. */
export interface DocImage {
  /** Google Docs inline-object id (write path). Absent in the offline mock. */
  objectId?: string
  /** Rendered width in points. */
  widthPt: number
  /** Rendered height in points. */
  heightPt: number
  /** Docs index of the inline-image element (one index wide). Lets the write path delete
   *  the image and re-insert it at a new size — the API has no in-place resize request. */
  docStart?: number
  /** Image source URL (sourceUri, else the short-lived contentUri). Required to re-insert
   *  at a smaller size; absent for some uploaded/pasted images, which then can't be resized. */
  uri?: string
}
...
export interface Paragraph {
  ...
  kind: ParagraphKind
  runs: TextRun[]
  /** Inline images anchored in this paragraph, if any. */
  images?: DocImage[]
  /** Left/right paragraph indents in points (the ruler's blue markers). 0/undefined = none. */
  indentStartPt?: number
  indentEndPt?: number
  /** True for an empty paragraph that carries a bottom border, i.e. the horizontal-line
   *  separator the page-break fix creates. It must never count as a blank line. */
  isRule?: boolean
...
/** Page margins in points (read from / written to the Docs API documentStyle). */
...
export interface DocModel {
  ...
  /** Page size in points (US-Letter 612×792 when absent). */
  pageWidthPt?: number
  pageHeightPt?: number
  /** Page margins in points; enables the wide-margins detector + fix. */
  margins?: PageMargins
}
...
/** A leading bullet glyph + space, for lists typed by hand rather than via Docs' list tool
 *  (the Docs API marks those paragraphs with a `bullet` field; these have only the glyph). */
export const BULLET_GLYPH = /^\s*[•·▪◦‣⁃∙*+-]\s+/

/** Strip a leading manual bullet glyph so table cells don't carry "• " into the grid. */
export function stripBulletGlyph(text: string): string {
  return text.replace(BULLET_GLYPH, '')
}

/** Indents at or below this (0.5in) are normal styling and left alone; above it wastes
 *  horizontal space and is reducible. Shared by the detector and both edit backends. */
export const INDENT_KEEP_PT = 36

/** Which sides of a paragraph carry a wasteful (wide) indent worth removing. */
export function wideIndentSides(p: Paragraph): { left: boolean; right: boolean } {
  return {
    left: (p.indentStartPt ?? 0) > INDENT_KEEP_PT,
    right: (p.indentEndPt ?? 0) > INDENT_KEEP_PT,
  }
}

/** A body paragraph whose left or right indent is wide enough to reclaim space. Headings,
 *  list items and breaks are skipped — their indents are structural, not wasted margin. */
export function hasWideIndent(p: Paragraph): boolean {
  if (p.kind !== 'normal' || paragraphText(p).trim().length === 0) return false
  const { left, right } = wideIndentSides(p)
  return left || right
}
 ...
 *  - an oversized chart image (largeImage)
 */
export const SAMPLE_DOC: DocModel = rebuildOffsets({
  ...
  defaultFontSize: 11,
  linesPerPage: 46,
  // US-Letter with wasteful 1.5in margins all round — trips the wide-margins detector.
  pageWidthPt: 612,
  pageHeightPt: 792,
  margins: { topPt: 108, bottomPt: 108, leftPt: 108, rightPt: 108 },
  paragraphs: [
    { id: pid(), kind: 'heading', runs: [run('Q3 Sustainability Report', { fontSize: 20, bold: true })] },
    {
      ...
      kind: 'normal',
      runs: [run('')],
      // ~468×360pt ≈ 56% of the printable page — trips the largeImage detector.
      images: [{ objectId: 'img-cover', widthPt: 468, heightPt: 360 }],
    },
    {
      id: pid(),
      kind: 'normal',
